...
GLvoid Framework::Timer(int value)
{
	...
	case CamMove:
		...
		case Up:
			...
			if ((instance->maze_collision() || instance->door_collision()) && !instance->creativemode)
			{
				instance->camerapos.x -= instance->camera.speed * instance->camera.cameraAt.x;
				instance->camerapos.z -= instance->camera.speed * instance->camera.cameraAt.z;
			}
...
{
	for (int i = 0; i < 20; ++i) {
		if (pow((m_ppObject[i + 32]->move_pos.x - camerapos.x), 2) + pow((m_ppObject[i + 32]->move_pos.z - camerapos.z), 2) + pow((m_ppObject[i + 32]->move_pos.y - camerapos.y), 2) < 0.04 && m_ppObject[i + 32]->exist == true && 크리에이티브모드 == false) {   //아이템 먹기
	...
	for (int i = 0; i < 3; ++i) {
		if (pow((m_ppObject[i + 7]->move_pos.x - camerapos.x), 2) + pow((m_ppObject[i + 7]->move_pos.z - camerapos.z), 2) + pow((m_ppObject[i + 7]->move_pos.y - camerapos.y), 2) < 0.04 && m_ppObject[i + 7]->exist == true) {   //아이템 먹기
	...
	if (!creativemode) {
		for (int i = 0; i < 20; ++i) {
			if (pow((m_ppObject[i + 12]->move_pos.x - camerapos.x), 2) + pow((m_ppObject[i + 12]->move_pos.z - camerapos.z), 2) + pow((m_ppObject[i + 12]->move_pos.y - camerapos.y), 2) < 0.01) {   //아이템 먹기
